...
for_cond:                                         ; preds = %for_body, %entry
  ...
  %for_cmp = fcmp olt double %for_i_load, 5.000000e+00
  br i1 %for_cmp, label %for_body, label %for_after

for_body:                                         ; preds = %for_cond
  %x = alloca double, align 8
  store double %for_i_load, ptr %x, align 8
  %load_sum = load double, ptr %sum, align 8
  %load_x = load double, ptr %x, align 8
  %addtmp = fadd double %load_sum, %load_x
  store double %addtmp, ptr %sum, align 8
  %reload_sum = load double, ptr %sum, align 8
  store double %reload_sum, ptr %for_result, align 8
...
for_after:                                        ; preds = %for_cond
  ...
  %load_sum2 = load double, ptr %sum, align 8
  %cmptmp = fcmp oeq double %load_sum2, 1.000000e+01
