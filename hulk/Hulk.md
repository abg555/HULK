...
Here `msg` is a new symbol that is defined _only_ within the expression that goes after `in`.
...
Variables (and identifiers in general) in HULK can be named with any sequence of alphanumeric characters, plus underscore `_`, but must _always_ begin with a letter (not a digit or `_`), hence the following are all valid identifiers:
...
The basic type relation in HULK is called _conforming_ (`<=`). A type `T1` is said to _conform to_ to another type `T2` (writen as `T1 <= T2`) if a variable of type `T2` can hold a value of type `T1` such that every possible operation that is semantically valid with `T2` is guaranteed to be semantically valid with `T1`.
...
The following rules provide an initial definition for the _conforming_ relationship. The formal definition is given in the section about [type semantics](/guide/type_semantics).
...
If a type inferer satisfies those constraints, we will say it is _sound_. This means that, for example, the simplest sound strategy for type inference is to infer types for all expressions and fail for all symbols. We will call this the _basic inference_ strategy.
...
> **Note:** Names like `_P1`, `_P2`, etc. in this section are **internally synthesized protocol names** used by the type inferrer. They are not intended as user-written identifiers in source code, and therefore they do not contradict the lexical rule that user identifiers must begin with a letter.
...
In order to implementing a protocol, a type doesn't necessarily have to match the exact signature of the protocol. Instead, method and type arguments are considered _contravariant_, and return values _covariant_. This means that arguments can be of the same type or higher, and the return values of the same type or lower than as defined in the protocol.
...
Since iterables are iterables, you can safely pass a vector as argument to method that expects an iterable:
...
The iterable protocols defined so far encapsulates the concept of making _a single iteration_ over the sequence of elements. In contrast, most collection types you will define allow for multiple iterations, even simultaneously, over the same sequence of elements.

To accomodate for this kind of behaviour, we can define an _enumerable_ protocol that simply provides one method to create an iterable for one specific iteration everytime that is needed:
...
Instead of a function, you can use a _macro_, which has a very similar syntax in HULK:
...
But this change makes macros exceedingly more powerful than functions in a lot of cases, for a few reasons. First, notice the use of the `*expr: Object` syntax, instead of the `expr: () -> Object`. Here the `*` denotes that this `expr` is **not** a regular argument, instead it is a special argument that refers to the code inside the brackets _after_ the macro invocation. Thus, you can use the following syntax:
...
This is the reason why you don't see `expr();` in the macro body, but `expr;`. That is, the body is not _executed_ but _interpolated_ inside the macro. This transpilation step makes macros often faster than functions because there is no extra overhead for passing arguments, however, you must be careful when thinking about the operational semantics of a macro especially where they differ from a regular function call.
...
In these cases, you can use the especial syntax `@symbol` to define a _symbolic argument_ in the macro, and then bind a specific symbol upon macro expansion.
